...
### 10-10. 쿼리 수 보정 조건의 처리 한계 재측정 결과 (세션 `de_caph2_20260918_1436`, 48회, 2026-09-18)

- 데이터 품질: k6 비정상 종료 0, InfluxDB 저장률 1.000, 시계 검사 2회 모두 드리프트 0.01% 이하
- 10-9 에서 고정한 설계대로 네 조건(REST·gRPC·GraphQL light·GraphQL 기존)을 **같은 세션 안에서** 풀 50:50 · rate 800/900/1000/1100 · 각 3회 측정했습니다
- 최대 달성 처리량 (3회 평균 [최소–최대], 조건별로 rate 를 올려도 평평해지는 값):

| 조건 | 최대 달성 iter/s | 평평해지는 rate | 병목 |
|---|---|---|---|
| REST (19쿼리) | 1,034 [1,024–1,048] | 1100 | API 측 대기 |
| gRPC (19쿼리) | 1,017 [995–1,046] | 1100 | API 측 대기 |
| GraphQL light (19쿼리) | 953 [924–957] | 1100 | API 측 대기 |
| GraphQL 기존 (24쿼리) | 839 [822–840] | 900 | API 측 대기 |

- 판정(10-9 고정 규칙 `H2_CAP_DIFF=0.05`):
  - **보정 효과 있음** — 24 → 19쿼리로 839 → 953 iter/s (+13.6%), 3회 범위 비중첩
  - **보정 후에도 GraphQL 의 처리 한계가 유의하게 낮음** — GraphQL light 953 [924–957] vs 낮은 쪽 기준 gRPC 1,017 [995–1,046] (−6.3%), 3회 범위 비중첩
- 중단 규칙 적용: 네 조건이 모두 목표 미달(달성률 95% 미만)이 되는 최저 rate 가 **1100 rps** 이므로 그 위로 올리지 않았습니다
- 병목은 네 조건 모두 **API 컨테이너 CPU** 입니다. 최대 달성 구간에서 API CPU 198~199%(제한 200%), 상한 도달 시간이 실행의 93~94% 인 반면 **DB CPU 는 128~142% 로 상한 도달 시간 0%** 였습니다
  - DB CPU 스로틀링은 조건별 차이가 있습니다: REST 0.7~3.4%, GraphQL light 0.3~10.8%, GraphQL 기존 12.1~17.8%, gRPC 3.0~28.5%. 스로틀링이 가장 큰 gRPC 가 처리 한계는 두 번째로 높으므로, 이 값들은 병목 판정을 뒤집지 않는 보조 지표로만 읽습니다
- **보정 전 값(10-8, 세션 `de_cap_20260918_0345`)은 비교를 위해 그대로 둡니다**: REST 1,015 / gRPC 983 / GraphQL(24쿼리) 821 iter/s. 두 세션 모두 풀 50:50 이며 세션 간 재현성은 REST +1.9%, gRPC +3.5%, GraphQL 기존 +2.2% 입니다. 아키텍처 간 비교는 세션 간 차이를 피하기 위해 **이 절(단일 세션) 값을 기준으로 삼습니다**
